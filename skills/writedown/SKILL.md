...
## 공개 수준 — 공개 vs 그냥 올림(잠금) 🔒

사이트 화면 노출을 두 단계로 고른다. ⚠️ GitHub 소스는 어느 쪽이든 **public** — 잠금은 "사이트 화면에서의 가림"일 뿐 실제 보안이 아니다(비밀번호 `0000`). **민감 정보는 잠금이어도 올리지 않는다.**

| 수준 | 의미 | 사이드바 | 비밀번호 게이트 |
|---|---|---|---|
| 🌐 **공개** | 누구나 바로 열람 | 상단, 정상 노출 | 없음 |
| 🔒 **그냥 올림(잠금)** | 올리되 화면에선 가림 | 하단 분리(`sidebar-locked`) + 하위목록 숨김 | 비밀번호(`0000`) |

- **현재 잠금 카테고리**: `회사`·`TIL`·`toyProject`·`개발노트`·`AI R&D`·`준비중`. **공개 카테고리**: `단어장`·메인.
- **초안·작업중**은 `준비중`(slug `/wip`, position 100)에 둔다 — 정리되면 알맞은 카테고리로 옮긴다.

**잠금 카테고리를 만들/유지하려면 두 곳을 맞춘다** (둘 다 해야 가려진다):
1. `vault/<카테고리>/_category_.json` — 라벨 끝에 `🔒`, `"className": "sidebar-locked"`, 하단 `position`(96~100대), `link.slug`는 ASCII(예 `/wip`). 빈 카테고리면 빌드가 깨지지 않게 placeholder 문서 1개(예 `소개.md`, `_` 접두사는 Docusaurus가 제외하니 쓰지 말 것)를 둔다.
2. `website/src/theme/Root.js`의 `PROTECTED` 정규식에 **폴더명과 slug를 둘 다** 추가(예: `…|준비중|wip`). 안 넣으면 비밀번호 게이트가 안 걸린다. (Root.js 변경은 `website/` 경로라 push 시 재배포된다.)

공개 카테고리는 위 두 처리를 **하지 않는다**(`sidebar-locked`·🔒·PROTECTED 등록 없이 일반 `position`).
